...
import { defaultProductImageUrl } from "@/lib/constants";
...
export function Product({ product, className, user }: Props) {
  return (
      ...
      <div className="flex w-full flex-col items-center justify-start">
        <div className="relative w-full overflow-hidden">
          ...
          <div className="absolute bottom-[2%] right-[2%] lg:hidden">
            <DynamicMobilePopover price={product.price} productTitle={product.productTitle} productId={product.id} />
          </div>
          ...
          <div className="absolute bottom-[3%] left-0 z-20 hidden w-full translate-y-[150%] flex-row items-center justify-around gap-2 ring-0 transition-all duration-300 group-hover:translate-y-0 lg:flex">
            {product.price ? (
              <AddButton productId={product.id} user={user} />
            ) : (
            ...
            <div className="flex flex-row items-center justify-between gap-3">
              {product.price ? (
                <ButtonWithIcon productId={product.id} user={user}>
